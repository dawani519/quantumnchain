  ...
  try {
    ...
    if (data.success) {
      ...
      userDetails.innerHTML = `
        ...
        <div class="user-detail">
          <div class="detail-label">Default Wallet Address:</div>
          <div class="detail-value">${data.data.wallet_address || "Not set"}</div>
        </div>
        <div class="user-detail">
          <div class="detail-label">Bitcoin Wallet:</div>
          <div class="detail-value">${data.data.bitcoin_wallet || "Not set"}</div>
        </div>
        <div class="user-detail">
          <div class="detail-label">Ethereum Wallet:</div>
          <div class="detail-value">${data.data.ethereum_wallet || "Not set"}</div>
        </div>
        <div class="user-detail">
          <div class="detail-label">USDT Wallet:</div>
          <div class="detail-value">${data.data.usdt_wallet || "Not set"}</div>
        </div>
        <div class="user-detail">
...
async function editUser(userId) {
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("bitcoinWallet").value = data.data.bitcoin_wallet || ""
      document.getElementById("ethereumWallet").value = data.data.ethereum_wallet || ""
      document.getElementById("usdtWallet").value = data.data.usdt_wallet || ""
...
function setupModalClosing() {
  ...
  const walletAddressesModal = document.getElementById("walletAddressesModal")
  ...
  for (let i = 0; i < spans.length; i++) {
    spans[i].onclick = function () {
      ...
      } else if (this.closest("#walletAddressesModal")) {
        walletAddressesModal.style.display = "none"
      }
    }
  }
  ...
  window.onclick = (event) => {
    if (event.target == userModal) {
      ...
      userDetailsModal.style.display = "none"
    } else if (event.target == walletAddressesModal) {
      walletAddressesModal.style.display = "none"
    }
  }
}
