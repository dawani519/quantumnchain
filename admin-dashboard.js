...
async function fetchDashboardStats() {
  try {
    const response = await fetch("http://localhost:8000/api/admin/dashboard-stats.php", {
      method: "GET",
      headers: {
    ...
    if (data.success) {
      ...
      document.getElementById("totalDeposits").textContent = `$${Number.parseFloat(data.data.totalDeposits).toFixed(2)}`
      document.getElementById("totalWithdrawals").textContent =
        `$${Number.parseFloat(data.data.totalWithdrawals).toFixed(2)}`
...
async function fetchRecentUsers() {
  try {
    const response = await fetch("http://localhost:8000/api/admin/users.php?limit=5", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("qc_admin_token")}`,
...
async function fetchRecentTransactions() {
  try {
    const response = await fetch("http://localhost:8000/api/admin/transactions.php?limit=5", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("qc_admin_token")}`,
